...
export default function ContactForm({ defaultMessage = '' }) {
  ...
  // Prefill runs client-side only: the page is prerendered without query
  // params, so setting this during render would cause a hydration mismatch.
  useEffect(() => {
    if (defaultMessage && messageRef.current && !messageRef.current.value) {
    ...
    <div>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <textarea ref={messageRef} id="message" name="message" required rows={5} placeholder="Describe a process you'd like to automate, a training need, or just tell us what's frustrating you." className={`${inputClass} placeholder:text-disabled`} />
        </div>
